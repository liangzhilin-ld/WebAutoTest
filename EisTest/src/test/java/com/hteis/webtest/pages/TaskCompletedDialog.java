...
public class TaskCompletedDialog extends HtmlPage{
	...
	public ArrayList<FlowStepData> getFlowStepData() throws Exception {
		...
		if (page.existByCss(".ngdialog-content table")) {

			for (HtmlRow row : page.findElementByCss(".ngdialog-content table").toHtmlTable().getRows()) {
	...
	public void verifyFlowStepData(FlowStepData[] expectedSteps) throws Exception{
		...
		for(int i = 0; i < expectedSteps.length; i++){
			...
			Assert.assertTrue(DateUtil.isCloseTime(actualStep.CreateDate, step.CreateDate), prefix + "处理时间不对");
